...
export const ItemCard = ({ name, description, image, price, stock, itemId, farmId, addText, action, noDetails }) => {
    return (
        ...
        <Card variant="outlined" sx={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", border:"3px solid rgba(0, 0, 0, 0.15)" }} >
            ...
            <CardActions>
                <Button 
                    ...
                    fullWidth 
                    sx={{ padding: [2, 2, 1] }} 
                    onClick={(itemId, farmId) => action({ name, description, image, itemId, stock, farmId })}>
                    {addText}
                </Button>
            </CardActions>
        </Card>
