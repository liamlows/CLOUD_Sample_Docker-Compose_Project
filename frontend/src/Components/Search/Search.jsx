...
const Search = ({ searchObject ,setSearchObject, justItem }) => {
    ...
    const handlePriceSearchChange = (priceChoice) => {
        ...
        handleSearchChange({minPrice: priceSelection.minPrice, 
            ...
            <Box sx={{ flexDirection: 'row', maxWidth: ["100%", "100%"], alignItems: "stretch" }}>
                {
                    filterOptions.map((option, index) => {
                        return <ToggleButton sx={{ ml: [1, 1, 0], mr: [1, 1, 1  ], mb: [1, 1, 2], paddingLeft: 1, justifyContent: "left", width: ["fit-content", "fit-content"] }}
                            key={option.label}
                            color="primary"
                        ...
                        >
                            <Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: [14, 16, 20] }, padding: 0 }} checked={searchObject.filters.includes(option.label)} color="success" />
